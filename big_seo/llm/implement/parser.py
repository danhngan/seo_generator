...
from big_seo.llm.indexer import (IParser
                                 )
from big_seo.common.lang import DocumentLang

from big_seo.llm.core.common import (Document, IDocumentCreator)
...
class OutlineDocumentCreator(IDocumentCreator):
    def create(self, **kwargs) -> Document:
        ...
        return OutlineDocumentDBType(header=header, content=content, doc_lang=doc_lang)
...
class WebPageParser(IParser):
    ...
    def __init__(self, start_tag='h1',
        ...
        self.heading_tag = set(['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])
    ...
    def _parse_recursive_structure(self, soup: BeautifulSoup, generations: list):
        for node in self._child_generation(soup):
            ...
            current = {'tag': node.name,
                       ...
                       'children': []}
            if self.check_if_heading_tag(node.name):
    ...
    def _get_main_content(self, soup: BeautifulSoup, thres: int = 0.5) -> BeautifulSoup:
        body = soup.find('body')
        if not body:
    ...
    def _child_generation(self, soup: BeautifulSoup):
        for child in soup.childGenerator():
            if child.name in ('section', 'div'):
                for sub in self._child_generation(child):
                    yield sub
            else:
                yield child

    def check_if_heading_tag(self, tag_name):
        if tag_name in self.heading_tag:
            return True
        else:
            return False
...
class UpToDateWebPageParser(WebPageParser):
    ...
    def _parse_recursive_structure(self, soup: BeautifulSoup, generations: list):
        for node in soup.childGenerator():
            ...
            if not node_name or node_name in self.exclude_tags:
                continue
            if self.check_if_heading_tag(node_name):
    ...
    def _get_heading_name(self, soup: BeautifulSoup):
        if not soup.name:
            return soup.name
        if self.check_if_heading_tag(soup.name):
            return soup.name
            ...
            if isinstance(soup_class, str) and soup_class == 'headingAnchor':
                ...
                if span and span.get_attribute_list('class'):
                    for span_class in span.get_attribute_list('class'):
                        if isinstance(span_class, str) and len(span_class) == 2 and self.check_if_heading_tag(span_class):
                            return span_class
        return soup.name
